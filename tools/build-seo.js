...
function buildJsonLd(a) {
    ...
    const headline = `${a.code} — ${a.name} (${a.nameTh})`;
    const keywords = [
        'cat personality',
        'MeowBTI',
        'cat MBTI',
        ...
        a.nameTh,
        a.code,
        'แบบทดสอบบุคลิกภาพแมว',
        'cat personality test',
    ];

    const graph = [
        {
            '@type': 'WebPage', // 2026 YMYL Trust Signal
            'name': headline,
            'description': a.aeoDescription || a.tagline,
            'lastReviewed': TODAY,
            'aspect': 'Psychology/Personality',
            'mainEntity': {
                '@type': 'CategoryCode',
                'codeValue': a.code,
                'name': a.name,
                'alternateName': a.nameTh
            }
        },
        {
            '@type': 'Article',
...
function aeoBlock(a) {
    if (!a.aeoDescription) return '';
    return [
        '    <!-- AEO: Answer-First Block (generated by tools/build-seo.js) -->',
        '    <div class="aeo-answer-block" style="border-left:4px solid #1F1410; padding:16px; margin:24px 0; background:#f9f9f9; border-radius:0 12px 12px 0;">',
        '        <p style="font-weight:700; font-size:1.1em; margin-bottom:8px;">Verdict Summary / สรุปความป่วน</p>',
        `        <p style="font-size:1.1em; line-height:1.5;">${a.aeoDescription}</p>`,
        `        <p style="font-size:1em; line-height:1.5; margin-top:8px; opacity:0.8; font-style:italic;">${a.thaiAeoDescription}</p>`,
        '    </div>'
    ].join('\n');
}
...
function rewriteHtmlForArchetype(a) {
    ...
    html = html.replace(/<title>.*?<\/title>/, `<title>${a.name} (${a.code}) — MeowBTI Cat Personality</title>`);
    html = html.replace(/<meta name="description" content=".*?">/, `<meta name="description" content="${a.aeoDescription || a.tagline}">`);

    // Match the existing JSON-LD block
    const reLd = /[ \t]*<!--\s*JSON-LD[^>]*-->[\s\S]*?<\/script>/;
    if (reLd.test(html)) {
        html = html.replace(reLd, ldBlock(a));
    } else {
        // If not found, insert in head
        html = html.replace('</head>', ldBlock(a) + '\n</head>');
    }

    // Inject AEO Block into <main> or after <body>
    const reAeo = /[ \t]*<!--\s*AEO[^>]*-->[\s\S]*?<\/div>/;
    if (reAeo.test(html)) {
        html = html.replace(reAeo, aeoBlock(a));
    } else {
        // Try to insert after <body> opening
